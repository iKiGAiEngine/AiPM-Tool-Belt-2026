...
### Core Logic
- **Spec Extractor**: Automates Division 10 specification extraction using a regex engine with AI enhancement (GPT-4o-mini) for section label review and project name suggestions. An opt-in **Detailed Spec Review** (checkbox on the upload screen, or run later from the results screen) reads each extracted section in full with GPT-4o and fills out a Short Order Form per material — manufacturer, model, material, size, finish, mounting, fire rating, quantity basis. Information the spec never gave is logged as an RFI, an assumption, or something to qualify in the proposal. Deterministic rules in `server/specDetailRules.ts` (unit-tested by `server/specDetailRules.test.ts`) flag what an estimator must not miss: multiple versions of the same material (e.g. two wall protection panel types or several visual display board types), fire-rated fire extinguisher cabinets, no-substitution language, missing manufacturers, unclear stainless grades, custom finishes, mock-ups and more. Results are stored on `spec_extractor_sections.detail_review` and exported as a four-tab Excel workbook (Estimator Summary, Short Order Form, Flags, Open Items & RFIs) covering every section extracted — downloadable on its own or bundled into the section ZIP.
